---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "vtb_elasticsearch_image_data Data Source - terraform-provider-vtb"
subcategory: ""
description: |-
  Данные образа из сервиса справочников для оформления заказа.
---

# vtb_elasticsearch_image_data (Data Source)

Данные образа из сервиса справочников для оформления заказа.

## Example Usage

```terraform
data "vtb_elasticsearch_image_data" "elastic_image" {
  os_version   = "1.8"
  distribution = "astra"
}
```

<!-- schema generated by tfplugindocs -->
## Schema

### Required

- `distribution` (String) Название дистрибутива (RHEL or Astra).
- `os_version` (String) Версия образа дистрибутива.

### Read-Only

- `ad_integration` (Boolean) Флаг для интеграции вычислительного экземпляра с Active Directory
- `geo_distribution` (Boolean) Флаг для использования геораспределения узлов кластера
- `on_support` (Boolean) Флаг означает, что продукт будет поддерживаться группой поддержки.
- `product_id` (String) Product ID из сервиса product_catalog.


